		...
		if(is_file($ini_file))
		{
			...
			foreach($arr_prod_tmp as $tag=>$val)
			{
				...
				foreach($val as $_k=>$_v)
				{
					...
					{
						$val[ $_k ]['MYSQL_SSL_CIPHER'] = "";
					}
                    elseif($tag == 'PROFILE' && !isset($_v['postgres_sslmode']))
                    {
                        $val[ $_k ]['postgres_sslmode'] = "N";
                    }
                    elseif($tag == 'PROFILE' && !isset($_v['postgres_sslrootcert']))
                    {
                        $val[ $_k ]['postgres_sslrootcert'] = "N";
                    }
                    elseif($tag == 'PROFILE' && !isset($_v['postgres_sslkey']))
                    {
                        $val[ $_k ]['postgres_sslkey'] = "N";
                    }
                    elseif($tag == 'PROFILE' && !isset($_v['postgres_sslcert']))
                    {
                        $val[ $_k ]['postgres_sslcert'] = "N";
                    }
                    elseif($tag == 'PROFILE' && !isset($_v['mssql_encrypt']))
                    {
                        $val[ $_k ]['mssql_encrypt'] = "N";
                    }
                    elseif($tag == 'PROFILE' && !isset($_v['mssql_trustservercertificate']))
                    {
                        $val[ $_k ]['mssql_trustservercertificate'] = "N";
                    }
                    elseif($tag == 'PROFILE' && !isset($_v['mssql_truststore']))
                    {
                        $val[ $_k ]['mssql_truststore'] = "";
                    }
                    elseif($tag == 'PROFILE' && !isset($_v['mssql_truststorepassword']))
                    {
                        $val[ $_k ]['mssql_truststorepassword'] = "";
                    }
                    elseif($tag == 'PROFILE' && !isset($_v['mssql_hostnameincertificate']))
                    {
                        $val[ $_k ]['mssql_hostnameincertificate'] = "";
                    }
				}
